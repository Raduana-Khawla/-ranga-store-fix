...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <button onclick="loadProductDetail(${product.id})" id="details-btn" class="btn btn-warning">Details</button></div>
      `;
...
const loadProductDetail = (productDetails) =>{
const url = `https://fakestoreapi.com/products/${productDetails}`;
fetch(url)
.then(res =>res.json())
...
const displayMealDetail = detail =>{
  ...
  <img src="${detail.image}" class="product-image" alt="...">
        <h3>${detail.title}</h3>
